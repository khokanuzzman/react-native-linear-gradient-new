# react-native-linear-gradient-new
###Note: Orginal package is : react-native-linear-gradient
Here i have changed `LinearGradientPackage` to `LinearGradientPackageNew` to avoid conflicting with some other package like `react-native-gradient-multi-slider`
...
```sh
yarn add react-native-linear-gradient-new
```

Or, using npm: `npm install react-native-linear-gradient-new`
